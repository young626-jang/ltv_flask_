        ...
        html, body {
            height: 100%; /* <-- 이 한 줄이 모든 문제 해결의 시작입니다. */
            margin: 0;
            padding: 0;
            overflow: hidden; /* <-- 전체 페이지에 불필요한 스크롤바가 생기는 것을 막아줍니다. */
        ...
        *, *::before, *::after {
        ...
        .pdf-viewer-column { 
            flex: 2; /* <-- 변경: 2/5 비율 (기존 40% 역할) */
            display: flex; 
            flex-direction: column; 
            transition: flex 0.3s ease; /* <-- 변경: width 대신 flex에 transition 적용 */
            background: #F2EDD7;
            border-radius: 12px;
            padding: 0;
        ...
        /* PDF 업로드 전 상태 - 드래그앤드롭만 보이는 컴팩트 모드 */
        .pdf-viewer-column.compact {
            flex: 1;
            min-width: 200px;
            /* max-width 제거하여 리사이즈 가능하게 함 */
        }
        
        .pdf-viewer-column.compact #upload-section {
            display: flex !important;
        }
        
        .pdf-viewer-column.compact #viewer-section {
            display: none !important;
        }
        .form-column { 
            flex: 3; /* <-- 변경: 3/5 비율 (기존 60% 역할) */
            height: 100%; 
            overflow-y: scroll; 
            transition: flex 0.1s ease; /* <-- 변경: width 대신 flex에 transition 적용 */
        ...
        @media (max-width: 768px) {
            ...
            /* 모바일에서도 리사이즈 바 표시하되 터치에 최적화 */
            .resize-bar {
                width: 100% !important;
                height: 12px; /* 터치하기 쉽도록 높이 확대 */
                cursor: row-resize;
            ...
            /* 모바일에서는 불필요한 라벨 제거 - placeholder로 충분 */
        ...
        @media (max-width: 576px) {
            ...
            /* 모든 컨테이너들 모바일에서 풀 너비 */
            [style*="background: #F2EDD7"],
            [style*="background: #6E6E6D"] {
                margin: 0 !important;
        ...
        @media (hover: none) and (pointer: coarse) {
            ...
            /* 모든 인터랙티브 요소 터치 최적화 */
            .form-control, .form-select, .btn, .md-btn, .md-ltv-btn {
                min-height: 44px;
        ...
        @media (max-width: 768px) {
            ...
            * {
            ...
            /* 모바일에서 호버 효과 제거하고 터치 효과로 대체 */
            .btn:hover, .form-control:hover, .md-btn:hover {
                transform: none;
                box-shadow: initial;
            }
            
            .btn:active, .md-btn:active {
        ...
        @media (max-width: 576px) {
            .mobile-label {
                display: none !important; /* 모바일에서 라벨 숨기기 */
                margin-bottom: 0.25rem;
                color: #495057;
                font-size: 0.8rem;
            }
        ...
        /* ================================== */
        /* ✅ 대출 항목 정렬을 위한 CSS 클래스 */
        /* ================================== */
        .loan-row-header, .loan-item {
            display: flex;
            align-items: center;
        }

        .loan-col {
            padding: 0 0.25rem; /* 좌우 여백 추가 */
        }

        .loan-col-drag { flex: 0 0 32px; }
        .loan-col-lender { flex: 1 1 30%; min-width: 120px; }
        .loan-col-max-amount { flex: 1 1 15%; min-width: 90px; }
        .loan-col-ratio { flex: 1 1 7%; min-width: 60px; }
        .loan-col-principal { flex: 1 1 15%; min-width: 90px; }
        .loan-col-status { flex: 1 1 12%; min-width: 80px; }
        .loan-col-action { flex: 0 0 40px; display: flex; justify-content: flex-end; }
        ...
        .kb-price-warning::placeholder {
            color: rgba(255, 255, 255, 0.7);
        }

        .kb-price-warning:focus {
        ...
        .ownership-transfer-warning::placeholder {
            color: rgba(255, 255, 255, 0.7);
        }

        .ownership-transfer-warning:focus {
            background-color: #ff5252 !important;
            box-shadow: 0 0 0 0.2rem rgba(255, 107, 107, 0.25);
        }

        /* 희망담보대부 지역 선택 버튼 스타일 */
        #hope-loan-region-buttons {
            display: none !important;
        }

        #hope-loan-region-buttons:not(.hidden) {
            display: flex !important;
        }

        .hope-loan-region-btn {
            flex: 1;
        ...
        .hope-loan-region-btn:hover {
            background-color: #e0e0e0;
            border-color: #555;
        }

        .hope-loan-region-btn.active {
            background-color: #9CC3D5;
            color: #0063B2;
            border-color: #9CC3D5;
        }

        .hope-loan-region-btn.active:hover {
            background-color: #8ab4c8;
            border-color: #8ab4c8;
        }

        /* 희망담보대부 체크박스 테두리 굵게 */
        #hope-collateral-loan {
            border: 2px solid #333 !important;
            cursor: pointer;
        }

        #hope-collateral-loan:focus {
